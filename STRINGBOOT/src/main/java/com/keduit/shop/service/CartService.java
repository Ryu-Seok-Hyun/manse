...
public class CartService {
  ...
  public List<CartDetailDTO> getCartDetail(String email) {
  ...
  // 현재 로그인 한 회원과 장바구니에 있는 상품을 저장한 회원이 같으면 true, 다르면 false
  @Transactional(readOnly = true)
  ...
  public Long orderCartItem(List<CartOrderDTO> cartOrderDTOList, String email) {
    ...
    //장바구니 페이지에서 전달 받은 주문상품 번호를 이용하여 주문 로직으로 전달할 orderDTO 객체를 생성함.
    ...
    // 장바구니에 담은 상품을 주문 하도록 로직을 가지고 있는 orderService.orders()를 호출
    Long orderId = orderService.orders(orderDTOList, email);

    // 주문한 상품은 장바구니 목록에서 제거 함.
    for (CartOrderDTO cartOrderDTO : cartOrderDTOList) {
      CartItem cartItem = cartItemRepository.findById(cartOrderDTO.getCartItemId())
          .orElseThrow(EntityNotFoundException::new);
      cartItemRepository.delete(cartItem);
